...
export function createSigningRequest(
  req: Omit<SigningRequest, "created_at" | "resolved_at" | "pdf_hash" | "attachment_id" | "rejection_reason">
): void {
  const db = getDb();
  db.prepare(
    `INSERT INTO signing_requests
      (id, document_id, document_title, document_text, author_user_id,
       signer_user_id, signer_email, signer_name, status, webhook_delivery_id, expires_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    req.id,
    req.document_id,
